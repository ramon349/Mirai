...
def run_epoch(data_loader, train_model, truncate_epoch, models, optimizers, args):
    ...
    exams = []
    """
    models['model'].transformer.transformer.transformer_layer_0.multihead_attention.query.register_forward_hook(getActivation('query')) 
    """ 
    ...
    if args.cluster_exams:
        ...
        exams = exam_set
        golds = gold_set
    #store_query_acti(activation,exams)
...
def store_query_acti(actis,exams): 
    df = pd.DataFrame({'exam_id':exams,'activations':actis['query']})
    df.to_csv("./predicted_activations.csv",index=False) 
